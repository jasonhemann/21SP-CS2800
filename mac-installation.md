...
6.0.14](https://www.virtualbox.org/wiki/Download_Old_Builds_6_0#DownloadVirtualBoxOldBuilds:VirtualBox6.0:~:text=VirtualBox%206.0.14%20(released%20October%2015%202119)). This is the VM we will be using to run ACL2s. We require version is 6.0.14 because we have tested with this version; other versions may cause installation problems later. The latest version 6.1 is not supported by vagrant, which we need, so certainly do not use it. (N.B. On recent versions of OSX the installation may fail the first time you try it for lack of sufficient permission.)
  ...
  3. Download, then install the [VirtualBox extensions for VirtualBox Version 6.0.14](https://download.virtualbox.org/virtualbox/6.0.14/Oracle_VM_VirtualBox_Extension_Pack-6.0.14.vbox-extpack). After downloading the file, you have to actually install the extensions. If you click on `Preferences > Extensions` you will see a green `+` icon that allows you to add extensions. Select the file you downloaded.
  ...
  6. Install [vagrant version 2.2.6](https://releases.hashicorp.com/vagrant/2.2.6/vagrant_2.2.6_x86_64.dmg). 
