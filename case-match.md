...
Pattern matching is a mechanism for checking a variable symbol against a pattern and deconstructing it into its constituent parts. We can think it as a more powerful version of `cond` in acl2. 
...
* Non-symbol atoms in the pattern match only themselves (e.g. (cons ‘a nil) does not match the pattern (cons B nil). Because ’a does not match B or C, it only matches A).
...
&               Matches anything (like a wild card).  & is not bounded 
			      and repeated occurrences of & in a pattern may match different structures.
